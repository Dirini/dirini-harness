...
이 하네스를 정리할 때 아래 생태계에서 구조적 영감을 받았습니다.

- **k-skill 계열 README들**: 설치 후 바로 실행 가능한 구조, 한국어 사용자 중심 설명 방식
- **AI Native Camp 계열 자료들**: 학습 흐름을 스킬과 루틴으로 묶는 방식
- **[anthropics/skills](https://github.com/anthropics/skills)**: Claude Code 스킬 설계 레퍼런스
- `docs/조민-하네스.md`: 이 레포의 사고 기준이 되는 10원칙 정리

일부 스킬은 제 로컬 환경에서 외부 스킬 팩을 통해 설치된 흔적을 바탕으로 포함되어 있습니다.
원저자나 출처가 더 명확해지면 이 섹션에 업데이트하겠습니다.
