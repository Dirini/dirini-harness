...
| `agents/writing-router.md` | 형식과 목적을 먼저 분류 | 필요 시 호출 |
...
`writing-router` → `writing-orchestrator` → `writing-intake` → `voice-profiler` → `background-researcher` → `argument-architect` → `draft-writer` → `output-evaluator`

- 먼저 형식과 목적을 분류하고
- 그다음 상위 orchestrator가 순서를 고정한 뒤
