...
`assignment-brainstorming` → `style-learning` → `background-researcher` → `draft-writer` → `output-evaluator` → `korean-spell-check`
...
`writing-intake` → `voice-profiler` → `background-researcher` → `draft-writer` → `output-evaluator`
